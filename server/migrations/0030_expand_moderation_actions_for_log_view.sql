-- Expand moderation action coverage for moderation log reads and add log-focused indexes.
...
CREATE TABLE moderation_actions_new (
    ...
    CONSTRAINT ck_moderation_actions_type CHECK (
        action_type IN ('mute', 'kick', 'ban', 'voice_kick', 'message_delete', 'warn')
    ),
...
CREATE INDEX IF NOT EXISTS idx_moderation_actions_log_guild_created
    ON moderation_actions(guild_id, created_at DESC, id DESC);

CREATE INDEX IF NOT EXISTS idx_moderation_actions_log_guild_action_created
    ON moderation_actions(guild_id, action_type, created_at DESC, id DESC);
